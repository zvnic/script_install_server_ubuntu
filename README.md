...
USER_PASSWORD=""                    # задать пароль (sudo будет спрашивать его)
SUDO_NOPASSWD=0                     # 1 = sudo без пароля через /etc/sudoers.d/<user>
```

> 🛡️ **sudo и пароль:** пользователь создаётся с `--disabled-password`, поэтому без пароля или правила `NOPASSWD` команда `sudo` работать не будет. Задайте `USER_PASSWORD` (рекомендуется) либо `SUDO_NOPASSWD=1`. Членство в группе `sudo` назначается всегда — в том числе уже существующему пользователю.
