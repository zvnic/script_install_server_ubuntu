...
GENERATE_SSH_KEY=1                  # если ключей нет вообще — сгенерировать пару на сервере
SSH_KEY_TYPE="ed25519"              # тип генерируемого ключа

# Пользователь создаётся без пароля (вход по ключу). Для рабочего sudo выберите одно:
USER_PASSWORD=""                    # пусто = пароль спросят интерактивно (рекомендуется)
SUDO_NOPASSWD=0                     # 1 = sudo без пароля через /etc/sudoers.d/<user>
```

> 🛡️ **sudo и пароль:** пользователь создаётся с `--disabled-password`, поэтому без пароля или правила `NOPASSWD` команда `sudo` работать не будет. По умолчанию (`USER_PASSWORD` пуст) скрипт **запросит пароль интерактивно** с подтверждением — в файле его хардкодить не нужно, в лог он не пишется. Для автоматизации можно задать `USER_PASSWORD` заранее или `SUDO_NOPASSWD=1`. Членство в группе `sudo` назначается всегда — в том числе уже существующему пользователю.
...
> 🔐 **Генерация ключа:** при `GENERATE_SSH_KEY=1` и полном отсутствии ключей скрипт создаёт пару `ed25519`, кладёт публичный ключ в `authorized_keys`, а путь к приватному выводит в лог. **Обязательно скачайте приватный ключ на свою машину и удалите его с сервера** (команда `scp ... && rm ...` печатается в логе).
